...
export default function About() {
  return (
    <Container narrow className="py-16">
      <p className="font-mono text-xs text-accent tracking-widest mb-4 animate-fade-up">ABOUT</p>
      <h1 className="font-display text-4xl font-bold mb-8 text-ink animate-fade-up-delay-1">
        From service floors and switchboards to the frontend.
      </h1>

      <p className="mb-4 text-lg text-ink-soft leading-relaxed">
      ...
      <p className="mb-12 text-lg text-ink-soft leading-relaxed">
      ...
      <Card>
        <p className="font-mono text-xs text-ink-soft tracking-widest mb-4">EDUCATION</p>
        <ul className="space-y-3 text-ink-soft">
          {education.map((item) => (
            <li key={item.label} className="flex justify-between gap-4 flex-wrap items-center">
              <span className="text-ink">{item.label}</span>
              <Badge tone={item.status === "completed" ? "accent" : "live"}>{item.status}</Badge>
            </li>
          ))}
        </ul>
      </Card>
